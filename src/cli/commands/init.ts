...
export async function runInit(opts: InitOptions): Promise<void> {
  ...
  if (scope.scope === "project") {
    ...
    // When gitignore is enabled, add .agents/.gitignore to the root .gitignore
    // so the generated file itself isn't committed to git.
    if (config.gitignore) {
      await ensureRootGitignoreEntry(scope.root);
    }
  }
...
const ROOT_GITIGNORE_COMMENT = "# .agents/.gitignore is generated by dotagents — don't commit it.";
const ROOT_GITIGNORE_ENTRY = ".agents/.gitignore";

async function ensureRootGitignoreEntry(projectRoot: string): Promise<void> {
