...
| `plugins` | table[] | `[]` | Plugin declarations for Claude, Cursor, Codex, Copilot, Grok, OpenCode, and Pi. |
...
Copilot MCP uses `.mcp.json` or an existing `.github/mcp.json` in project scope
and `$COPILOT_HOME/mcp-config.json` (default `~/.copilot/mcp-config.json`) globally.
