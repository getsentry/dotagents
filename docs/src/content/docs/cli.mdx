...
| `agents` | string[] | `[]` | Agent targets: `claude`, `cursor`, `codex`, `vscode`, `opencode`, `augment` |
| `subagents` | table[] | `[]` | Custom subagent declarations for Claude, Cursor, Codex, OpenCode, and Augment |
...
| `path` | string | No | Exact directory for a named Git skill, or an existing contained recursive discovery root for a Git/local wildcard; use `"."` for the complete source root. Well-known sources do not support `path`. |
...
Operates on the current Git repository. Commands run from a subdirectory
discover the repository root and use its `agents.toml`.
