...
import type { PluginManifest, PluginMarketplace } from "./plugin-schema.js";
...
function marketplaceOutputs(
  ...
  if (codexPlugins.length > 0) {
    outputs.push({
      ...
      content: stableJson(codexMarketplace(projectRoot, codexPlugins)),
    });
  }
...
function codexMarketplaceEntry(
  ...
  const entry: PluginMarketplace["plugins"][number] = {
    name: plugin.name,
    source: {
...
function opencodeModules(plugin: PluginDeclaration): string[] {
  ...
  return candidates.filter((path) => existsSync(join(plugin.pluginDir, path)));
}
