...
`);
  ...
  it.each(["claude", "copilot", "cursor", "codex", "grok", "opencode", "pi"] as const)(
...
`,
      ...
      if (target === "claude" || target === "copilot" || target === "cursor") {
        const marketplaceDir = target === "claude"
          ? ".claude-plugin"
          : target === "copilot"
            ? ".github/plugin"
            : ".cursor-plugin";
  ...
  it("installs user-scope plugins and writes global runtime projections", async () => {
    ...
    try {
      ...
      await writeFile(
        scope.configPath,
        `version = 1
agents = ["claude", "codex", "copilot", "opencode", "pi"]
