...
Tasks:
  ...
  plugin-opencode  Assert generated OpenCode module and debug config load
...
async function runSyncRepair() {
  ...
  rmSync(join(projectDir, ".opencode", "plugins", "qa-tools.ts"), { force: true });
  runCli(["sync"]);
  assertFile(".mcp.json");
...
async function runOpenCodePluginProof() {
  await installAndAssert();
  const config = execFileSync("opencode", ["debug", "config"], {
    cwd: projectDir,
    env: fixtureEnv,
    encoding: "utf-8",
  });
  assertFile(".opencode/plugins/qa-tools.ts");
  assertFileIncludes(".opencode/plugins/qa-tools.ts", "Generated by dotagents");
  assertFileIncludes(".opencode/plugins/qa-tools.ts", "../.agents/plugins/qa-tools/opencode/plugin.ts");
  if (!config.includes("qa-tools.ts") || !config.includes("dotagents-plugin-proof") || !config.includes("DOTAGENTS_OPENCODE_PLUGIN_EXECUTION_PROOF")) {
    throw new Error("OpenCode debug config did not include generated plugin proof command");
  }
}
...
function assertPluginOutputs() {
  ...
  assertFileIncludes(".agents/plugins/qa-tools/.codex-plugin/plugin.json", '"commands": "./commands"');
  assertFileIncludes(".agents/plugins/qa-tools/.codex-plugin/plugin.json", '"agents": "./agents"');
